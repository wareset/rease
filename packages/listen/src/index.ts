...
function _listen<Type extends keyof GlobalEventHandlersEventMap>(
  E: Window | Document | HTMLElement | SVGElement,
  type: `${Type}${'' | `-${string}`}`,
  ...
  if (TYPE) {
    ...
    if (TYPE === 'resize') {
      ...
      // else E.dispatchEvent(createCustomEvent('resize'))
      callback.call(E, defineTargetForCustomEvent(E, createCustomEvent(TYPE)))
    }
  } else {
...
export const listen: typeof _listen =
